...
class flowAPI
{
	...
	function __construct() {
		...
		$this->test_flow = get_option('test-banner-system');

		$this->upload_dir = wp_upload_dir();
		$this->upload_url = $this->upload_dir['baseurl'] . '/bannersystem/flow/';

		if (!empty($this->test_flow) && $this->test_flow == 'live'){
			$this->flow_url_pago = 'https://www.flow.cl/app/kpf/pago.php';
			$this->order["MedioPago"] = get_option('medio-pago-banner-system');
			$this->flow_comercio = get_option('form-email-comercio-banner-system');
			$this->flow_tipo_integracion = get_option('acceso-banner-system');
			$this->pem = get_option('certificado-banner-system');
		}else{
			$this->flow_url_pago = 'http://flow.tuxpan.com/app/kpf/pago.php';
			$this->order["MedioPago"] = get_option('sandbox-medio-pago-banner-system');
			$this->flow_comercio = get_option('sandbox-form-email-comercio-banner-system');
			$this->flow_tipo_integracion = get_option('sandbox-acceso-banner-system');
			$this->pem = get_option('sandbox-certificado-banner-system');
		}
		...
		$this->order["Status"] = "";
		$this->order["Error"] = "";
		$this->url_pay =  get_option('banner_system_url_payment');
		$this->flow_url_exito = $this->url_pay . '?flowsu=sucess';
	 ...
	 */
	public function new_order($orden_compra, $monto,  $concepto, $email_pagador)
	{
		...
		return array('status' => true, 'url' => $this->flow_url_pago, 'parameters' => $this->_flow_pack());
	}
	...
	public function read_confirm()
	{
		...
		$this->flow_log($print, "read_confirm");
		...
		if(isset($params['kpf_pagador'])) {
			...
			$this->flow_log("Lee Pagador: " . $params['kpf_pagador'], "read_confirm");
		}
	 ...
	 */
	public function build_response($result = true)
	{
	...
	public function read_result()
	{
		...
		parse_str($data, $params);
		$print = print_r($params,true);
		$this->flow_log($print, "read_result");
		if (!isset($params['s'])) {
	...
	public function flow_log($message, $type)
	{
		...
		$file = fopen($this->file_logflow , "a+");
		fwrite($file, "[".date("Y-m-d H:i:s.u")." - $type ] ".$message . PHP_EOL);
		fclose($file);
	}
	...
	{
		try {
			$fp = fopen($this->upload_url.$this->pem, "r");
	...
	private function flow_sign($data)
	{
		...
		if(! openssl_sign($data, $signature, $priv_key_id)) {
			...
			return array('status' => false, 'message' => 'NotSign');
		}
	...
	private function _flow_pack()
	{
		...
		if (!$hConcepto) $hConcepto = htmlentities($this->order["Concepto"], ENT_COMPAT | ENT_HTML401, 'UTF-8');
		if (!$hConcepto) $hConcepto = htmlentities($this->order["Concepto"], ENT_COMPAT | ENT_HTML401, 'ISO-8859-1');
	...
	public function updataConfirm($front = false)
	{
		$this->flow_log("write{$this->order['Status']}", "updataConfirm");

		$concepto = $this->order['OrdenNumero'];
		$key_banner = explode(' N',$concepto);
		$key_banner = $key_banner[0];

		$id = explode('-',$concepto);
		$id = (int)$id[1];
		$namebanner = "status_order_$key_banner";

		$src_pay = "banner_system_".$key_banner."_banner_src_pay";
		$src_default = $key_banner."_bannersystem";
		$src_default = get_option($src_default);
		...
		if (isset($this->order['OrdenNumero']) && $process != 'confirm' && $front){
			...
			update_option($src_pay,$src_default);
			$this->flow_log("update confirm front", "updataConfirm");
			//send messages
			$this->_sendEmailFlow($id,$namebanner,true);
		}
		if (isset($this->order['OrdenNumero']) && $process == 'confirm' && $front){
			update_option($src_pay,$src_default);
			delete_option($namebanner);
	...
	private function _sendEmailFlow($id,$name,$front=false)
	{
		$meta = get_post_meta($id,'order_banner_system',true);

		if (!empty($meta) && isset($meta[$name])){

			$suject = __('Banner rentado ','bannersystem') . home_url();

			if (isset($meta[$name]['password'])){
				$message = sprintf( __('Hola, hemos recibido tu pedido. Para terminar de activar el banner debes de ingresar a la zona de miembros desde este enlace %s nombre de usuario: %s contraseña:%s', 'bannersystem' ), wp_login_url(), $meta[$name]['email'],  $meta[$name]['password']);
			}else{
				$message = __('Hola, hemos recibido tu pedido. Ingresa a tu zona de miembros para subirlo. ','bannersystem') . wp_login_url();
			}

			$send = wp_mail( $meta[$name]['email'], $suject, $message );
